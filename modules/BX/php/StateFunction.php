...
class StateFunctionRow extends \BX\Action\BaseActionRow
{
    ...
    {
        $this->functionCalls = json_encode(\BX\Meta\extractAllPropertyValues($functionCalls));
        if (strlen($this->functionCalls) > self::MAX_JSON_SIZE) {
            throw new \BgaSystemException('BUG! functioncalls is too long: ' . strlen($this->functionCalls));
        }
    }

    public function getFunctionCalls()
    {
        $jsonRow = json_decode($this->functionCalls, true);
